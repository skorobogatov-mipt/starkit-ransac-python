...
def generate_mobius(
    ...
    for i in range(n_points):
        vector_in_circle_plane = (
            np.cos(angles[i]) * zero_angle_vector + np.sin(angles[i]) * v1
        )
        p = radius * vector_in_circle_plane
        half_width_vector = half_width * (
            vector_in_circle_plane * np.cos(angles[i] * orientation / 2)
            + normal * np.sin(angles[i] * orientation / 2)
        )
        one_end = p + half_width_vector
        another_end = p - half_width_vector
        mobius_points[i, :] = (
            another_end * values[i] + one_end * (1 - values[i]) + center
        )

    if n_uniform_ponts > 0:
        points_uniform = box_center + np.random.uniform(
            low=-box_size / 2, high=box_size / 2, size=(n_uniform_ponts, 3)
        )
