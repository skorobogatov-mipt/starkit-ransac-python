...
def generate_circle(circle: Circle3D, noise_sigma: float = 0.05, n_points=1000):
    ...
    points = np.vstack([np.cos(angles), np.sin(angles), angles * 0]).T
    points *= radius

    # 2) rotate circle
    points = rotate_from_axis_to_axis(points, [0, 0, 1], normal)
