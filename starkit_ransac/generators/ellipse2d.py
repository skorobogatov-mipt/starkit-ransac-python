...
def generate_ellipse2d(ellipse: Ellipse2D, noise_sigma=0.05, n_points=1000):
    thetas = np.linspace(-np.pi, np.pi, n_points)
    a = ellipse.major_radius
    b = ellipse.minor_radius
    points = np.array([a * np.cos(thetas), b * np.sin(thetas)]).T
    rot = ellipse.rotation
    rotation_matr = np.array([[np.cos(rot), -np.sin(rot)], [np.sin(rot), np.cos(rot)]])
