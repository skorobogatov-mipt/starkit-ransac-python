...
def generate_mobius_mesh(mobius_strip, color=np.array([1, 0, 0]), resolution=100):
    ...
    for theta in angles:
        ...
        half_width_vector = half_width * (
            vector_in_circle_plane * np.cos(theta * orientation / 2)
            + normal * np.sin(theta * orientation / 2)
        )
        one_end = p + half_width_vector
        another_end = p - half_width_vector
        for j in range(n_width + 1):
            mobius_points[i, j, :] = (
                another_end * j / n_width + one_end * (n_width - j) / n_width + center
            )
        i += 1
