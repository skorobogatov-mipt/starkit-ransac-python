...
class MobiusStrip(AbstractSurfaceModel):
    ...
    def fit_model(self, points: NDArray):
        ...
        start_vector = (
            np.cos(circle_angle) * radial_dir - np.sin(circle_angle) * tangent_dir
        )
    ...
    def calc_distances(self, points: NDArray) -> NDArray:
        ...
        objective_values = self.objective_function_multi_target(
            theta_grid, points_in_mobius_coordinates
        )
    ...
    def calc_distance_one_point(self, point: NDArray) -> float:
        ...
        objective_values = objective_function_array(
            theta_grid, point_in_mobius_coordinates
        )
        ...
        bracket = (theta_grid[best_index] - step, theta_grid[best_index] + step)
    ...
    def objective_function_multi_target(self, theta_values, target_points):
        ...
        b_vectors = self.compute_b_vector_multi_target(theta_values, target_points)

        residual_vectors = b_vectors + optimal_v[..., None] * a_vectors[None, :, :]
