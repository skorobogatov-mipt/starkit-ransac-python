...
class Ellipse2D(AbstractSurfaceModel):
    def __init__(
        ...
        if polynomial is not None:
            ...
            if polynomial.shape != (5,):
                raise ValueError(
                    "Ellipse polynomial must contain 5 numbers: Ax^2 + Bxy^ + Cy^2 + Dx + Ey = 1"
                )

        if (
            center is not None
            and radius_1 is not None
            and radius_2 is not None
            and rotation is not None
        ):
            polynomial = self.polynomial_from_rotation_and_radii(
                rotation, radius_1, radius_2, center
            )
    ...
    def fit_model(self, points: ArrayLike):
        ...
        self.rotation, self.major_radius, self.minor_radius, self.center = (
    ...
    @staticmethod
    def polynomial_from_rotation_and_radii(rotation, radius_1, radius_2, center):
        # https://en.wikipedia.org/wiki/Ellipse#Parametric_representation
    ...
    def calc_distances(self, points: NDArray) -> NDArray:
        ...
        x = points[:, 0]
        y = points[:, 1]
        poly_values = np.dot(poly, np.array([x**2, x * y, y**2, x, y]))
