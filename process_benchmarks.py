...
def main():
    ...
    for benchmark in benchmark_data["benchmarks"]:
        ...
        if is_benchmark_starkit(benchmark):
            ...
            print(benchmark["stats"]["mean"])
        elif is_benchmark_scuf(benchmark):
            n_iter_scuf[test] += 1
            total_time_scuf[test] += benchmark["stats"]["mean"]
    ...
    collected_shapes = []
    for test in target_tests:
        if n_iter_stransac[test] == 0 or n_iter_pyransac[test] == 0:
            continue

        shape = test.removeprefix("test_")
        collected_shapes.append(shape)
        stransac_avgs.append(total_time_starkit[test] / n_iter_stransac[test])

        # if n_iter_scuf[test] != 0:
        #     scuf_avgs.append(
        #         total_time_scuf[test]/n_iter_scuf[test]
        #     )

        if n_iter_pyransac[test] != 0:
            pyransac_avgs.append(total_time_pyransac[test] / n_iter_pyransac[test])

    w = 0.4
    stransac_bar = np.arange(len(collected_shapes))
    pyransac_bar = stransac_bar + w
    scuf_bar = pyransac_bar + w
    print(pyransac_avgs)
    plt.bar(stransac_bar, stransac_avgs, w, label="starkit_ransac", color=(0, 1, 0))
    plt.bar(pyransac_bar, pyransac_avgs, w, label="pyransac", color=(1, 0, 0))
    # plt.bar(
    #         scuf_bar,
    #         scuf_avgs,
    #         w,
    ...
    #         color=(1,0,0)
    # )
    plt.xticks(stransac_bar + 3 * w / 2, collected_shapes, fontsize=24)
